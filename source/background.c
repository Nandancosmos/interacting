...
int background_derivs(
  ...
  if (pba->has_scf == _TRUE_){
    dy[pba->index_bi_Omega_phi_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*(3.*(pvecback[pba->index_bg_w_tot]+cos_scf(pba,y[pba->index_bi_theta_phi_scf])) + 0.5*pba->beta_scf*(1-cos_scf(pba,y[pba->index_bi_theta_phi_scf])));
      
    dy[pba->index_bi_theta_phi_scf] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      ((0.5*pba->beta_scf*exp(y[pba->index_bi_Omega_phi_scf]) - 3.0)*sin_scf(pba,y[pba->index_bi_theta_phi_scf])+y[pba->index_bi_y_phi_scf]);
